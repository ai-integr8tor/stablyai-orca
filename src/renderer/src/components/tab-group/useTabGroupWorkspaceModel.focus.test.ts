...
describe('useTabGroupWorkspaceModel floating worktree commands', () => {
  ...
  it('opens a local terminal when no web runtime session owns the floating worktree', async () => {
    mocks.createWebRuntimeSessionTerminal.mockResolvedValue(false)
  ...
  it('routes a floating terminal create through the active web runtime session', async () => {
    ...
    expect(mocks.createWebRuntimeSessionTerminal).toHaveBeenCalled()
  ...
  it('opens a local browser tab when no web runtime session owns the floating worktree', async () => {
    mocks.createWebRuntimeSessionBrowserTab.mockResolvedValue(false)
    const state = storeBox.state as Record<string, unknown>
    ...
    expect(mocks.createBrowserTab).toHaveBeenCalledWith(
      FLOATING_TERMINAL_WORKTREE_ID,
      'https://example.com',
      expect.objectContaining({ focusAddressBar: true, targetGroupId: FLOATING_GROUP_ID })
    )
  })
})
