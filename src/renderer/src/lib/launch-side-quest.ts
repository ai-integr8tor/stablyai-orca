...
export function launchSideQuest(args: {
  ...
  if (args.agent === 'codex' && worktree && !repo?.connectionId) {
    ...
    const tab = initialState.createTab(args.worktreeId, groupId, undefined, {
      launchAgent: 'codex',
    ...
    return { status: 'started', groupId, terminalTabId: tab.id }
  }
  ...
  return { status: 'started', groupId, terminalTabId: launched.tabId }
}
