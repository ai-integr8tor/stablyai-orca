...
const mocks = vi.hoisted(() => ({
  ...
  waitForAgentTabInputReady: vi.fn(),
  toastError: vi.fn()
}))
...
vi.mock('./agent-paste-draft', () => ({
  waitForAgentTabInputReady: mocks.waitForAgentTabInputReady
}))
...
describe('startTerminalSideQuest', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.waitForAgentTabInputReady.mockReturnValue(Promise.resolve(true))
  })
  ...
  it('binds a cleaned selection to the new terminal tab', () => {
    ...
    expect(mocks.waitForAgentTabInputReady).toHaveBeenCalledWith({
      tabId: 'side-tab',
      agent: 'codex',
      timeoutMs: 30_000
    })
