...
export function openHostLogicalClient(host: HostProfile, onLog: ConnectionLogSink): RpcClient {
  ...
  const logical = createStableLogicalRpcClient(
    connect(host.endpoint, host.deviceToken, host.publicKeyB64, { deviceName, onLog }),
  ...
  const endpointLifecycle = startMobileEndpointLifecycle(logical, host, onLog, deviceName)
